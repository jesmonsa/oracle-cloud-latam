# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Variables - Bastion Service + Webservers Privados                          ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
variable "subnet_privada_ad1_cidr" {
  description = "CIDR para subnet privada en AD1 (webservers)"
  type        = string
  default     = "10.0.1.0/24"
}

variable "subnet_privada_ad2_cidr" {
  description = "CIDR para subnet privada en AD2 (webservers)"
  type        = string
  default     = "10.0.2.0/24"
}
...
variable "bastion_cidr_permitidos" {
  description = "CIDRs permitidos para conectarse al Bastion (tu IP/32 recomendado)"
  type        = list(string)
  default     = ["0.0.0.0/0"]
}

variable "bastion_ttl_segundos" {
  description = "TTL máximo de sesiones Bastion en segundos"
  type        = number
  default     = 10800
}
...
variable "habilitar_nsg" {
  description = "Usar Network Security Groups (recomendado)"
  type        = bool
  default     = true
}
