# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Variables - Load Balancer + Alta Disponibilidad Multi-AD                   ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
variable "subnet_publica_ad1_cidr" {
  description = "CIDR para subnet pública en AD1 (webservers)"
  type        = string
  default     = "10.0.1.0/24"
}

variable "subnet_publica_ad2_cidr" {
  description = "CIDR para subnet pública en AD2 (webservers)"
  type        = string
  default     = "10.0.2.0/24"
}

variable "subnet_lb_cidr" {
  description = "CIDR para subnet regional del Load Balancer"
  type        = string
...
variable "lb_bandwidth_min_mbps" {
  ...
  validation {
    condition     = var.lb_bandwidth_min_mbps >= 10 && var.lb_bandwidth_min_mbps <= 8000
    error_message = "Bandwidth mínimo debe estar entre 10 y 8000 Mbps."
  }
}
...
variable "lb_bandwidth_max_mbps" {
  ...
  validation {
    condition     = var.lb_bandwidth_max_mbps >= 10 && var.lb_bandwidth_max_mbps <= 8000
    error_message = "Bandwidth máximo debe estar entre 10 y 8000 Mbps."
  }
}
