# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Variables — Arquitectura 11: WAF + DNS Zone                               ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
variable "subnet_privada_cidr" {
  description = "CIDR subnet privada (webserver)"
  type        = string
  default     = "10.0.1.0/24"
}
...
variable "dns_zone_name" {
  description = "Nombre de la zona DNS (dominio). Puede ser un dominio de prueba."
  type        = string
...
# ─── WAF ──────────────────────────────────────────────────────────────────────
variable "waf_modo" {
  description = "Modo del WAF: 'Detection' (solo logs) o 'Active' (bloquea)"
  type        = string
  default     = "Detection"

  validation {
    condition     = contains(["Detection", "Active"], var.waf_modo)
    error_message = "Valores: Detection o Active"
  }
}
