# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Main - Almacenamiento Compartido (File Storage Service NFS)                ║
# ║  Arquitectura: LB + Privados + Bastion + NAT + NFS compartido             ║
# ║                                                                            ║
# ║  Cambios clave vs Arquitectura 04:                                         ║
# ║    - File Storage Service (FSS) con Mount Target en subnet privada AD1    ║
# ║    - NFS montado automáticamente en ambos webservers via cloud-init       ║
# ║    - NSG NFS habilitado (puertos 111, 2048-2050)                          ║
# ║    - Contenido compartido accesible en /var/www/html/shared               ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
module "nsgs" {
  ...
  habilitar_nsg_nfs  = true   # ← NFS habilitado para File Storage Service
...
# El FSS se crea en AD1 y es accesible desde ambas subnets privadas via VCN
module "filesystem" {
