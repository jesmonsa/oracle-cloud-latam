module "red" {
  ...
  habilitar_nat_gateway = true # Requisito porque los webservers estarán en privada
...
# Subnet Pública (Solo para el LB y Bastion)
resource "oci_core_subnet" "publica" {
  ...
  # Si usamos NSG, la subnet confía en ellos y no requiere SL restrictivas
  security_list_ids = []
  freeform_tags     = local.tags_comunes
}

# Subnet Privada (Para los dos webservers)
