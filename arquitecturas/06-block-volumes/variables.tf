...
variable "ocpus_webserver" {
  description = "Cantidad de OCPUs para el Webserver"
  type        = number
  default     = 1
}

variable "memoria_webserver_gb" {
  description = "Cantidad de Memoria RAM (GB) para el Webserver"
  type        = number
  default     = 8
}

variable "tamano_block_volume_gb" {
  description = "Tamaño en GB del disco adicional (Block Volume) para cada webserver"
  type        = number
  default     = 50
  validation {
    condition     = var.tamano_block_volume_gb >= 50
    error_message = "El Block Volume debe ser de al menos 50 GB."
  }
}

variable "imagen_os" {
  description = "OCID de la imagen del sistema operativo (Oracle Linux 8+ recomendado)"
  type        = string
  default     = ""
}
