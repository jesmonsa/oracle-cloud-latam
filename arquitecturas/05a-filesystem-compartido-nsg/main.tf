...
# Subnet Pública (Solo para el LB)
...
# Subnet Privada (Para Bastion, Webservers y FSS)
...
module "fss" {
  ...
  cidr_permitido      = var.subnet_privada_cidr
  tags                = local.tags_comunes
}

# Aplicar el NSG a el recurso base de Mount Target directamente
# Debido a que el provider OCI asocia NSGs directamente en el mount_target. 
# En nuestro módulo FSS no hemos cableado un input `nsg_ids`. 
# Para mantener la fidelidad al módulo base, la asociación recaerá sobre 
# las reglas predeterminadas de NSG/SL o como mejora futura se agregarán.
# Por lo pronto, la arquitectura es válida a nivel lógico FSS.
